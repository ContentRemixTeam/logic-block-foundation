...
export function useMastermindPhaseOne() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [stateResult, resourceResult, proposalResult, contextResult, keyResult] = await Promise.all([
        ...
        db.from('ai_planner_task_proposals').select('proposal_id,task_text,task_description,why_this_task,done_enough,evidence_target,suggested_date,priority,status,approved_task_id').eq('user_id', auth.user.id).order('created_at', { ascending: false }).limit(10),
  ...
  const pendingProposal = useMemo(() => proposals.find((proposal) => proposal.status === 'pending') ?? null, [proposals]);
