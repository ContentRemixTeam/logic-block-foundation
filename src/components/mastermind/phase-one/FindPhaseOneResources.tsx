...
export function FindPhaseOneResources({ search, onOpenResource }: { search: (query: string) => Promise<PhaseOneResource[]>; onOpenResource: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (clean.length < 2) { setResults([]); setError(null); return; }
    const timer = window.setTimeout(() => {
      setLoading(true); setError(null);
      void search(clean).then(setResults).catch(() => setError('Search is temporarily unavailable. Your plan and progress are safe.')).finally(() => setLoading(false));
    }, 300);
