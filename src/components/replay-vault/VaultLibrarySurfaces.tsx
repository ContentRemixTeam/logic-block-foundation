  ...
  return (
    <section aria-labelledby="vault-library-heading" className="space-y-4" data-vault-library>
          ...
          {(items as BrowseItem[]).map((item) => (
            <li key={item.resourceId}>
              <Card className="h-full min-w-0">
                <CardHeader><div className="flex flex-wrap gap-2"><Badge variant="outline">{item.category}</Badge>{item.publishedAt && <Badge variant="secondary"><Clock className="mr-1 h-3 w-3" aria-hidden="true" />Recent</Badge>}</div><CardTitle className="break-words text-lg">{item.title}</CardTitle><CardDescription>{item.questionCount} approved question{item.questionCount === 1 ? '' : 's'} · {formatCompactTime(item.durationSeconds)}</CardDescription></CardHeader>
                <CardContent className="grid gap-2 sm:grid-cols-2">
                  <Button type="button" className="min-h-11 w-full" onClick={() => onOpen({ resourceId: item.resourceId, title: item.title, startSeconds: null, questionId: null, momentId: null })}><PlayCircle className="mr-2 h-4 w-4" aria-hidden="true" />Watch full replay</Button>
                  <Button type="button" variant="outline" className="min-h-11 w-full" disabled={busy === item.resourceId} onClick={() => void saveReplay(item.resourceId)}>{busy === item.resourceId ? <RotateCw className="mr-2 h-4 w-4 animate-spin motion-reduce:animate-none" aria-hidden="true" /> : <Bookmark className="mr-2 h-4 w-4" aria-hidden="true" />}Save full video</Button>
                </CardContent>
              </Card>
            </li>
          ))}
      ...
      {loadState === 'ready' && surface === 'saved' && (
        <ul className="space-y-3">
          {(items as SavedItem[]).map((item) => <li key={item.bookmarkId}><Card><CardHeader><div className="flex flex-wrap gap-2"><Badge>{item.targetKind === 'replay' ? 'Video' : 'Moment'}</Badge><Badge variant="outline">{item.category}</Badge></div><CardTitle className="text-lg">{item.title}</CardTitle><CardDescription>{item.label}{item.targetKind !== 'replay' ? ` · ${formatCompactTime(item.startSeconds)}` : ''}</CardDescription></CardHeader><CardContent className="flex flex-wrap gap-2">{item.targetKind === 'replay' ? <Button type="button" className="min-h-11" onClick={() => onOpen({ resourceId: item.resourceId, title: item.title, startSeconds: null, questionId: null, momentId: null })}>Open saved video</Button> : <Button type="button" className="min-h-11" onClick={() => onOpen({ resourceId: item.resourceId, title: item.title, startSeconds: item.startSeconds, questionId: item.targetKind === 'question' ? item.targetId : null, momentId: item.targetKind === 'moment' ? item.targetId : null })}>Open saved moment</Button>}<Button type="button" variant="outline" className="min-h-11" disabled={busy === item.bookmarkId} onClick={() => void remove(item.bookmarkId)}><Trash2 className="mr-2 h-4 w-4" aria-hidden="true" />Remove everywhere</Button></CardContent></Card></li>)}
        </ul>
      )}
