...
import { DollarSign, Plus, CalendarIcon, Pencil, TrendingUp, TrendingDown } from 'lucide-react';
...
export function SalesGoalTrackerWidget({ cycleId, cycleStartDate, cycleEndDate }: SalesGoalTrackerWidgetProps) {
  ...
  if (!cycleId) {
    return (
      <WidgetSection title="Sales Goal" icon={<DollarSign className="h-5 w-5" />} elevated>
    ...
    <WidgetSection title="Sales Goal" icon={<DollarSign className="h-5 w-5" />} elevated>
      ...
      {!isLoading && !revenueGoal && (
        <div className="text-center py-4">
          ...
          <p className="text-muted-foreground text-sm mb-3">Set a revenue goal to track sales</p>
      ...
      {!isLoading && revenueGoal && paceData && (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Progress value={paceData.progress} className="h-2" />
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            <span>{salesData?.salesCount ?? 0} sales</span>
            {salesData && salesData.salesCount > 0 && (
              <span>
                ${Math.round(paceData.totalRevenue / salesData.salesCount).toLocaleString()} avg
              </span>
            )}
          </div>
          ...
          <div className={cn(
            ...
            paceData.status === 'ahead' && "bg-green-500/10 text-green-700 dark:text-green-400",
            paceData.status === 'on-track' && "bg-yellow-500/10 text-yellow-700 dark:text-yellow-400",
            paceData.status === 'behind' && "bg-red-500/10 text-red-700 dark:text-red-400",
          )}>
