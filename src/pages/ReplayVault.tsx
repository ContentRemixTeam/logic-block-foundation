...
export default function ReplayVault() {
  ...
  const resolvePlayback = useCallback(async (nextTarget: PlaybackTarget, options: { recovery?: boolean; deepLink?: boolean } = {}) => {
    ...
    if (!options.recovery && playback?.resourceId === nextTarget.resourceId && playback.momentId === nextTarget.momentId && playback.questionId === nextTarget.questionId && Number.isFinite(playback.startSeconds)) {
