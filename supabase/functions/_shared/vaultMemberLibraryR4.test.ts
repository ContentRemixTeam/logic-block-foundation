...
Deno.test('library mapper chooses fixed RPCs and clamps pagination', () => {
  ...
  assertEquals(mapped?.args, { p_user_id: userId, p_portal_resource_id: 'replay-1', p_after_index: 100_000, p_limit: 100 });
...
Deno.test('caller identity, scope and time authority are rejected before RPC', async () => {
  for (const forged of [{ userId: 'other' }, { user_id: 'other' }, { scopes: ['vault'] }, { accessScopes: ['vault'] }, { timestamp: '2020-01-01' }, { asOf: '2020-01-01' }]) {
...
Deno.test('authorized requests use authenticated user only and RPC failures stay generic', async () => {
  let current = fixture();
  let response = await current.handler(request({ action: 'questions', resourceId: 'replay-1', offset: 2, limit: 60 }));
