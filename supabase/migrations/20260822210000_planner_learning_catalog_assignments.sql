...
CREATE TABLE IF NOT EXISTS public.curriculum_cycle_assignments (
  ...
  CHECK (assignment_status <> 'active' OR confirmed_at IS NOT NULL),
  CHECK (
...
BEGIN
  ...
  SELECT encode(digest(convert_to(string_agg(
      concat_ws('|', i.stable_item_key, i.item_state, i.stage, i.milestone_key,
        i.item_role, i.item_order::text, i.title, i.intended_output,
        coalesce(i.action_prompt, ''), coalesce(i.evidence_prompt, ''),
        i.teacher_display_name, i.attribution_text, i.source_system,
        i.source_native_id, i.source_provenance, coalesce(i.provenance_sha256, ''),
        coalesce(i.canonical_resource_id::text, ''), coalesce(i.transcript_version_id::text, ''),
        coalesce(i.playback_attempt_id::text, ''), coalesce(i.publication_sha256, ''),
        i.transcript_qa_state, i.provenance_qa_state, i.rights_qa_state,
        i.privacy_qa_state, i.edit_qa_state, i.caption_qa_state,
        i.playback_qa_state, i.action_qa_state, i.evidence_qa_state,
        coalesce(i.qa_receipt_sha256, ''), i.required_capability), E'\n' ORDER BY i.stable_item_key
    ), 'UTF8'), 'sha256'), 'hex')
...
BEGIN
  ...
  IF OLD.assignment_status = 'pending_confirmation' AND NEW.assignment_status = 'active' THEN
    IF (to_jsonb(NEW) - ARRAY['assignment_status', 'confirmed_at']::text[])
       <> (to_jsonb(OLD) - ARRAY['assignment_status', 'confirmed_at']::text[]) THEN
...
BEGIN
  ...
  IF FOUND AND v_active.assignment_id IS NOT NULL THEN
    ...
    END IF;
    IF p_rebuild_diff IS NULL OR jsonb_typeof(p_rebuild_diff) <> 'object'
       OR p_rebuild_diff = '{}'::jsonb THEN
      RAISE EXCEPTION 'rebuild requires an explicit nonempty diff before confirmation';
    END IF;
    v_status := 'pending_confirmation';
    v_diff_sha256 := encode(digest(convert_to(p_rebuild_diff::text, 'UTF8'), 'sha256'), 'hex');
  ELSE
  ...
  INSERT INTO public.curriculum_cycle_assignments(
    ...
    catalog_version_id, context_key, assignment_version, assignment_status,
    ...
    p_planner_receipt_id, p_catalog_version_id, p_context_key,
    v_assignment_version, v_status, p_supersedes_assignment_id,
    p_rebuild_diff, v_diff_sha256, p_created_by,
...
DECLARE
  ...
  v_capability_reason text;
BEGIN
  IF p_expected_rebuild_diff_sha256 IS NULL
     OR p_expected_rebuild_diff_sha256 !~ '^[0-9a-f]{64}$'
     OR btrim(coalesce(p_confirmed_by, '')) = '' THEN
    RAISE EXCEPTION 'exact rebuild diff receipt and confirmer are required';
  END IF;
  ...
  END IF;
  IF v_pending.rebuild_diff_sha256 <> p_expected_rebuild_diff_sha256 THEN
  ...
  UPDATE public.curriculum_cycle_assignments
     SET assignment_status = 'active', confirmed_at = v_now
  ...
  RETURN jsonb_build_object(
    ...
    'rebuild_diff_sha256', v_pending.rebuild_diff_sha256,
    'confirmed_at', v_now,
    ...
    SELECT 1
     ...
     WHERE ai.assignment_id = v_assignment.assignment_id
       AND (
         ...
         OR ai.canonical_resource_id <> ci.canonical_resource_id
         OR ai.transcript_version_id <> ci.transcript_version_id
         OR ai.playback_attempt_id <> ci.playback_attempt_id
         OR ai.publication_sha256 <> ci.publication_sha256
         OR EXISTS (
           SELECT 1 FROM public.curriculum_catalog_item_revocations r
            WHERE r.catalog_item_id = ci.catalog_item_id
         )
       )
  ) THEN
    RETURN jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION public.create_curriculum_cycle_assignment(uuid,uuid,uuid,uuid,uuid,text,uuid[],uuid,jsonb,text)
  TO service_role;
REVOKE ALL ON FUNCTION public.confirm_curriculum_assignment_rebuild(uuid,text,text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.confirm_curriculum_assignment_rebuild(uuid,text,text)
  TO service_role;
...
COMMENT ON TABLE public.curriculum_catalog_versions IS
  'Versioned Planner Learning catalog authority; publication and supersession are explicit transitions.';
...
COMMENT ON TABLE public.curriculum_cycle_assignments IS
  'Frozen owner/cycle/Planner-receipt assignments. Rebuilds require a separate diff confirmation boundary.';
