...
BEGIN
  ...
  UPDATE public.cycle_success_path_snapshots SET confirmed_stage=p_stage,current_milestone_id=p_milestone_id,current_milestone_title=v_milestone.label,confirmed_at=now(),curriculum_version='mastermind-curriculum-v1',updated_at=now() WHERE user_id=v_user AND cycle_id=p_cycle_id;
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, auth, pg_temp AS $$
DECLARE v_user uuid := auth.uid(); v_task uuid; v_action uuid; v_stage text; v_current_milestone text; v_expected_key text;
BEGIN
  ...
  SELECT confirmed_stage,current_milestone_id INTO v_stage,v_current_milestone FROM public.cycle_success_path_snapshots WHERE user_id=v_user AND cycle_id=p_cycle_id AND confirmed_stage IS NOT NULL AND confirmed_at IS NOT NULL;
  ...
  ON CONFLICT (user_id,stable_key) DO UPDATE SET task_id=EXCLUDED.task_id,exact_move=EXCLUDED.exact_move,capacity_mode=EXCLUDED.capacity_mode,done_enough=EXCLUDED.done_enough,evidence=EXCLUDED.evidence,scheduled_date=EXCLUDED.scheduled_date,updated_at=now()
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, auth, pg_temp AS $$
DECLARE v_user uuid:=auth.uid(); v_action public.mastermind_success_path_actions%ROWTYPE; v_stage text; v_id uuid; v_support text;
BEGIN
  ...
  SELECT * INTO v_action FROM public.mastermind_success_path_actions WHERE action_id=p_action_id AND user_id=v_user;
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='42501', MESSAGE='That action is unavailable.'; END IF;
  SELECT confirmed_stage INTO v_stage FROM public.cycle_success_path_snapshots WHERE user_id=v_user AND cycle_id=v_action.cycle_id;
