...
AS $function$
  WITH input AS MATERIALIZED (
    ...
    SELECT s.id, s.transcript_version_id, s.transcript_text_private, s.starts_at_ms, s.ends_at_ms,
    ...
    SELECT r.portal_resource_id, h.id moment_id, r.title, r.product_title, r.category_title,
      r.resource_type, h.transcript_text_private, h.starts_at_ms, h.ends_at_ms,
      (r.duration_ms / 1000)::integer duration_seconds, h.rank
    ...
    SELECT c.portal_resource_id, c.moment_id, NULL::uuid question_id, c.title, c.product_title,
      ...
      'matches transcript'::text reason, c.duration_seconds, c.rank
