...
BEGIN
  ...
  IF p_status = 'active' AND p_tier = 'lifetime' AND p_ends_at IS NOT NULL THEN
    RAISE EXCEPTION 'Lifetime Planner access cannot have an end date';
  END IF;

  IF p_status = 'active' AND p_tier IN ('monthly', 'annual') AND p_ends_at IS NULL THEN
