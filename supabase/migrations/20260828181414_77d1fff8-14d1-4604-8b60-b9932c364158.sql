...
BEGIN
  ...
  END IF;
  before_hash := encode(extensions.digest(to_jsonb(v)::text, 'sha256'), 'hex');
  ...
  INSERT INTO public.replay_editorial_review_events(
    ...
    'transcript', v.id, before_hash, encode(extensions.digest(to_jsonb(v)::text, 'sha256'), 'hex'),
...
CREATE OR REPLACE FUNCTION public.replay_transcript_content_hash(p_version uuid) RETURNS text LANGUAGE sql STABLE SET search_path=pg_catalog,public AS $$ SELECT encode(extensions.digest(coalesce(string_agg(s.segment_index||':'||s.starts_at_ms||':'||s.ends_at_ms||':'||regexp_replace(btrim(s.transcript_text_private),E'\\s+',' ','g'),E'\n' ORDER BY s.segment_index),''),'sha256'),'hex') FROM public.replay_transcript_segments s WHERE s.transcript_version_id=p_version $$;
...
BEGIN PERFORM public.replay_assert_actor(actor);rid:=(j->>'resource_id')::uuid;IF NOT EXISTS(SELECT 1 FROM public.mastermind_portal_resources WHERE id=rid)THEN RAISE EXCEPTION 'resource not found';END IF;SELECT state INTO prior FROM public.replay_publication_authority WHERE resource_id=rid FOR UPDATE;IF prior IS NOT NULL AND prior<>'DRAFT'THEN RAISE EXCEPTION 'only DRAFT resources may be reimported';END IF;
 ...
 IF raw~*'(PRIVATE_SENTINEL|[[:alnum:]._%+-]+@[[:alnum:].-]+\\.[A-Za-z]{2,})'THEN RAISE EXCEPTION 'private transcript sentinel';END IF;rh:=encode(extensions.digest(raw,'sha256'),'hex');sh:=encode(extensions.digest(jsonb_build_object('system',ss,'native_id',sn,'version',sv,'raw_sha256',rh,'metadata',j#>'{source,metadata}')::text,'sha256'),'hex');md:=(j#>>'{media,duration_ms}')::bigint;
 ...
 mh:=encode(extensions.digest(jsonb_build_object('system',ss,'native_id',mn,'version',mv,'byte',j#>>'{media,byte_sha256}','dropbox',j#>>'{media,dropbox_content_hash}','size',(j#>>'{media,size_bytes}')::bigint,'duration',md,'decode',j#>>'{media,decode_report_sha256}','range',j#>>'{media,range_report_sha256}','seek',j#>>'{media,seek_report_sha256}')::text,'sha256'),'hex');ih:=encode(extensions.digest(ss||E'\n'||sn||E'\n'||sv||E'\n'||mn||E'\n'||mv,'sha256'),'hex');ph:=encode(extensions.digest(sh||mh||ih||rid,'sha256'),'hex');
 INSERT INTO public.replay_ingestion_runs(run_kind,source_system,collector_version_sha256,source_snapshot_sha256,record_manifest_sha256,record_count,status,completed_at,created_by,package_sha256)VALUES('content_bound_import',ss,encode(extensions.digest('sql-r2','sha256'),'hex'),ph,ph,2,'accepted',now(),actor,ph)RETURNING id INTO runid;
