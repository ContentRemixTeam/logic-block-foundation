...
BEGIN
  ...
  SELECT string_agg(x.v, E'\n' ORDER BY x.v COLLATE "C") INTO ident
  FROM (
...
REVOKE ALL ON FUNCTION public.replay_import_launch_batch(jsonb, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.replay_import_launch_batch(jsonb, text) FROM anon, authenticated;
