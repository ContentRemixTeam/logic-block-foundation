-- Generalize protected Mastermind media playback so curriculum lessons can
-- reuse the Vault Dropbox transport without granting monthly members Vault access.
...
AS $$
  SELECT *
    FROM public.resolve_mastermind_media_playback(
      p_user_id, p_email, p_resource_id, 'vault', p_question_id, p_moment_id, p_preview, p_as_of
    );
$$;
...
COMMENT ON FUNCTION public.resolve_mastermind_media_playback(uuid,text,text,text,uuid,uuid,boolean,timestamptz)
  IS 'Shared Dropbox-backed protected media resolver for Mastermind curriculum, current replays, and annual/lifetime Replay Vault playback.';
