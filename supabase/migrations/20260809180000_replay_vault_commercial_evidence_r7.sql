...
CREATE TABLE IF NOT EXISTS public.replay_vault_commercial_deliveries (
  ...
  signature_timestamp bigint CHECK(signature_timestamp IS NULL OR signature_timestamp>0),
...
CREATE TABLE IF NOT EXISTS public.replay_vault_commercial_delivery_attempts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  canonical_delivery_id uuid NOT NULL REFERENCES public.replay_vault_commercial_deliveries(id) ON DELETE RESTRICT,
  provider text NOT NULL CHECK(provider=lower(trim(provider)) AND provider<>''),
  provider_delivery_id text NOT NULL CHECK(trim(provider_delivery_id)<>''),
...
CREATE INDEX IF NOT EXISTS replay_vault_delivery_attempts_canonical_idx
  ON public.replay_vault_commercial_delivery_attempts(canonical_delivery_id,verified_at,id);
...
DO $$ DECLARE n text; BEGIN
  FOREACH n IN ARRAY ARRAY['replay_vault_commercial_deliveries','replay_vault_purchase_contributions',
    'replay_vault_commercial_delivery_attempts','replay_vault_purchase_lifecycle_evidence',
    'replay_vault_commercial_quarantine','replay_vault_commercial_resolutions']
  LOOP
...
INSERT INTO public.replay_vault_commercial_deliveries(provider,provider_delivery_id,event_type,order_id,transaction_id,
  ...
  signature_sha256,signature_timestamp,signature_verified,effective_at,requested_expires_at,outcome,error_class,source_legacy_event_id,receipt)
SELECT w.provider,w.event_id,w.event_type,
  ...
  w.normalized_email,w.product_id,w.price_id,w.payload_sha256,NULL,NULL,true,w.effective_at,w.requested_expires_at,
...
CREATE OR REPLACE FUNCTION public.replay_vault_recompute_entitlement_r7(p_email text,p_as_of timestamptz)
RETURNS jsonb LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE e public.replay_vault_entitlements%ROWTYPE; winner public.replay_vault_purchase_contributions%ROWTYPE;
  decision_at timestamptz:=coalesce(p_as_of,clock_timestamp());
BEGIN
  ...
  SELECT x.* INTO winner
  FROM public.replay_vault_purchase_contributions x
  WHERE x.normalized_email=p_email
    AND x.contribution_starts_at<=decision_at
    AND (x.contribution_expires_at IS NULL OR decision_at<x.contribution_expires_at)
    AND NOT EXISTS(
      SELECT 1 FROM public.replay_vault_purchase_lifecycle_evidence l
      WHERE l.purchase_contribution_id=x.id
        AND l.lifecycle_type IN ('expiration','refund','chargeback','immediate_revocation')
        AND l.effective_at<=decision_at)
  ORDER BY CASE x.entitlement_tier WHEN 'lifetime' THEN 3 WHEN 'annual' THEN 2 ELSE 1 END DESC,
    x.contribution_starts_at DESC,x.created_at DESC,x.id DESC LIMIT 1;
  IF FOUND THEN
    ...
    VALUES(p_email,winner.entitlement_tier,'active',winner.contribution_starts_at,winner.contribution_expires_at,
      winner.provider,winner.order_id,winner.purchase_effective_at,decision_at,NULL,'validated')
  ...
  ELSIF e.id IS NOT NULL THEN
    UPDATE public.replay_vault_entitlements SET status='revoked',revoked_at=decision_at,
      last_transition_at=greatest(last_transition_at,decision_at),commercial_evidence_state='validated',updated_at=clock_timestamp()
    WHERE id=e.id;
  END IF;
...
CREATE OR REPLACE FUNCTION public.apply_replay_vault_commercial_event_r7(
  ...
  p_payload_sha256 text,p_signature_sha256 text,p_signature_timestamp bigint,p_effective_at timestamptz,p_access_expires_at timestamptz DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE
...
BEGIN
  IF v_provider='' OR v_event_id='' OR position('@' in v_email)<=1
    ...
    OR p_payload_sha256!~'^[0-9a-f]{64}$' OR p_signature_sha256!~'^[0-9a-f]{64}$' OR p_signature_timestamp IS NULL
    OR p_signature_timestamp<=0 OR p_effective_at IS NULL
  ...
  IF FOUND THEN
    INSERT INTO public.replay_vault_commercial_delivery_attempts(canonical_delivery_id,provider,provider_delivery_id,
      signature_timestamp,signature_sha256)
    VALUES(v_delivery.id,v_provider,v_event_id,p_signature_timestamp,p_signature_sha256);
    v_semantic_equal:=v_delivery.payload_sha256=p_payload_sha256 AND v_delivery.event_type=p_event_type
      AND v_delivery.order_id IS NOT DISTINCT FROM v_order_id
      AND v_delivery.transaction_id IS NOT DISTINCT FROM v_transaction_id
      AND v_delivery.lifecycle_parent_order_id IS NOT DISTINCT FROM v_parent_order
      AND v_delivery.lifecycle_parent_transaction_id IS NOT DISTINCT FROM v_parent_transaction
      AND v_delivery.normalized_email=v_email AND v_delivery.product_id=p_product_id AND v_delivery.price_id=p_price_id
      AND v_delivery.effective_at=p_effective_at
      AND v_delivery.requested_expires_at IS NOT DISTINCT FROM p_access_expires_at;
  ...
  IF NOT FOUND THEN
    ...
    INSERT INTO public.replay_vault_commercial_deliveries(provider,provider_delivery_id,event_type,order_id,transaction_id,
      lifecycle_parent_order_id,lifecycle_parent_transaction_id,normalized_email,product_id,price_id,payload_sha256,signature_sha256,signature_timestamp,
    ...
    VALUES(v_provider,v_event_id,p_event_type,v_order_id,v_transaction_id,v_parent_order,v_parent_transaction,v_email,
      p_product_id,p_price_id,p_payload_sha256,p_signature_sha256,p_signature_timestamp,true,p_effective_at,p_access_expires_at,
      'rejected_unmapped','unmapped_product',v_receipt) RETURNING id INTO v_delivery_id;
    INSERT INTO public.replay_vault_commercial_delivery_attempts(canonical_delivery_id,provider,provider_delivery_id,
      signature_timestamp,signature_sha256)
    VALUES(v_delivery_id,v_provider,v_event_id,p_signature_timestamp,p_signature_sha256);
    RETURN v_receipt;
  END IF;
  ...
  IF v_is_purchase THEN
    ...
    IF FOUND THEN
      ...
      IF NOT v_semantic_equal THEN
        ...
        INSERT INTO public.replay_vault_commercial_deliveries(provider,provider_delivery_id,event_type,order_id,transaction_id,
          normalized_email,product_id,price_id,payload_sha256,signature_sha256,signature_timestamp,signature_verified,effective_at,requested_expires_at,
        ...
        VALUES(v_provider,v_event_id,p_event_type,v_order_id,v_transaction_id,v_email,p_product_id,p_price_id,
          p_payload_sha256,p_signature_sha256,p_signature_timestamp,true,p_effective_at,p_access_expires_at,
          'commercial_conflict','transaction_evidence_conflict',v_receipt) RETURNING id INTO v_delivery_id;
        INSERT INTO public.replay_vault_commercial_delivery_attempts(canonical_delivery_id,provider,provider_delivery_id,
          signature_timestamp,signature_sha256)
        VALUES(v_delivery_id,v_provider,v_event_id,p_signature_timestamp,p_signature_sha256);
        RETURN v_receipt;
      ...
      v_receipt:=jsonb_build_object('success',true,'replayed',true,'status','applied','purchaseContributionId',v_existing.id)
        ||public.replay_vault_recompute_entitlement_r7(v_email,clock_timestamp());
      INSERT INTO public.replay_vault_commercial_deliveries(provider,provider_delivery_id,event_type,order_id,transaction_id,
        normalized_email,product_id,price_id,payload_sha256,signature_sha256,signature_timestamp,signature_verified,effective_at,requested_expires_at,
        outcome,receipt)
      VALUES(v_provider,v_event_id,p_event_type,v_order_id,v_transaction_id,v_email,p_product_id,p_price_id,
        p_payload_sha256,p_signature_sha256,p_signature_timestamp,true,p_effective_at,p_access_expires_at,'replayed_purchase',v_receipt) RETURNING id INTO v_delivery_id;
      INSERT INTO public.replay_vault_commercial_delivery_attempts(canonical_delivery_id,provider,provider_delivery_id,
        signature_timestamp,signature_sha256)
      VALUES(v_delivery_id,v_provider,v_event_id,p_signature_timestamp,p_signature_sha256);
      RETURN v_receipt;
    ...
    VALUES(v_delivery_id,v_provider,v_event_id,p_event_type,v_order_id,v_transaction_id,v_email,p_product_id,p_price_id,
      p_payload_sha256,p_signature_sha256,p_signature_timestamp,true,p_effective_at,p_access_expires_at,'applied',v_receipt);
    INSERT INTO public.replay_vault_commercial_delivery_attempts(canonical_delivery_id,provider,provider_delivery_id,
      signature_timestamp,signature_sha256)
    VALUES(v_delivery_id,v_provider,v_event_id,p_signature_timestamp,p_signature_sha256);
    ...
    RETURN v_receipt||public.replay_vault_recompute_entitlement_r7(v_email,clock_timestamp());
  END IF;
  ...
  IF FOUND AND p_effective_at<v_purchase.purchase_effective_at THEN
    RAISE EXCEPTION 'invalid replay vault commercial event' USING ERRCODE='22023';
  END IF;
  IF NOT FOUND OR v_purchase.order_id<>v_parent_order OR v_purchase.normalized_email<>v_email
    ...
    INSERT INTO public.replay_vault_commercial_deliveries(provider,provider_delivery_id,event_type,
      lifecycle_parent_order_id,lifecycle_parent_transaction_id,normalized_email,product_id,price_id,payload_sha256,
      signature_sha256,signature_timestamp,signature_verified,effective_at,requested_expires_at,outcome,error_class,receipt)
    VALUES(v_provider,v_event_id,p_event_type,v_parent_order,v_parent_transaction,v_email,p_product_id,p_price_id,
      p_payload_sha256,p_signature_sha256,p_signature_timestamp,true,p_effective_at,p_access_expires_at,
      'rejected_transition','exact_parent_purchase_not_found',v_receipt) RETURNING id INTO v_delivery_id;
    INSERT INTO public.replay_vault_commercial_delivery_attempts(canonical_delivery_id,provider,provider_delivery_id,
      signature_timestamp,signature_sha256)
    VALUES(v_delivery_id,v_provider,v_event_id,p_signature_timestamp,p_signature_sha256);
    RETURN v_receipt;
  ...
  INSERT INTO public.replay_vault_commercial_deliveries(id,provider,provider_delivery_id,event_type,
    lifecycle_parent_order_id,lifecycle_parent_transaction_id,normalized_email,product_id,price_id,payload_sha256,
    signature_sha256,signature_timestamp,signature_verified,effective_at,requested_expires_at,outcome,receipt)
...
BEGIN
  ...
  FROM public.replay_vault_purchase_contributions c
  WHERE c.normalized_email=v_email AND c.contribution_starts_at<=p_as_of
  ...
  ORDER BY CASE c.entitlement_tier WHEN 'lifetime' THEN 3 WHEN 'annual' THEN 2 ELSE 1 END DESC,
    c.contribution_starts_at DESC,c.created_at DESC,c.id DESC LIMIT 1;
...
-- Forward hardening for inherited trigger helper: fixed lookup path and owner-only ACL.
...
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog AS $$ BEGIN
  ...
  RETURN NEW;
END $$;
REVOKE ALL ON FUNCTION public.replay_forbid_generated_question_publish() FROM PUBLIC,anon,authenticated,service_role;
...
REVOKE ALL ON FUNCTION public.apply_replay_vault_commercial_event_r7(text,text,text,text,text,text,text,text,text,text,text,text,bigint,timestamptz,timestamptz)
  FROM PUBLIC,anon,authenticated;
REVOKE ALL ON FUNCTION public.reconcile_replay_vault_unmapped_event_r7(uuid,text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.apply_replay_vault_commercial_event_r7(text,text,text,text,text,text,text,text,text,text,text,text,bigint,timestamptz,timestamptz) TO service_role;
...
COMMENT ON TABLE public.replay_vault_commercial_delivery_attempts IS 'Append-only evidence for every verified timestamp and signature attempt, separate from canonical payload identity.';
